...
export const attendaceHistory = ({history}) => {
  return (
            ...
            <tbody>
                 ...
                 {history.length === 0 ? (
                  <tr>
                   ...
                   history.map((record)=>{
                      const dayType = getTypeDisplay(record);
                    return(
                      <tr key={record.id || record.id}>
                        <td className="px-6 py-4 font-medium text-slate-900">{format(new Date(record.date), 'MM dd, yyyy')}</td>
                        ...
                        <td className="px-6 py-4">{dayType.label !== '' ? <span className={`badge ${dayType.label}`}> {dayType.label} </span>:'-'}</td>

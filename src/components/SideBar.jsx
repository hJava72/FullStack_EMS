...
const SideBar = () => {
    ...
    const sidebarcontent = (
        <>
            ...
            {/* User Profile Card */}
            {userName && (
                <div className='mx-5 mt-4 mb-1 p-3 rounded-lg bg-white/3 border border-white/4'>
                    <div className='flex items-center gap-3'>
            ...
            {/* Section Label */}
               <div className='px-5 pt-5 pb-2'>
                   <p className='text-[10px] text-slate-500 uppercase tracking-[0.2em] font-semibold  px-5'>Navigation</p>
               </div>
            ...
            <div className='flex-1 px-3 space-y-0.5 overflow-y-auto'>
                {navItems.map((item) => {
                    ...
                    return (
                        <Link key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-all duration-150 relative ${isActive ? "bg-indigo-500/12 text-indigo-300" : "text-slate-300 hover:text-white"}`}>
                            {isActive && 
                                <div className='absolute left-0 top-1/2 translate-y-1/2 w-[3px] h-5 bg-indigo-500 rounded-r-full' />
                            }
                            <item.icon className={`w-[17px] h-[17px] shrink-0 ${isActive ? "text-indigo-500" : "text-slate-400 group-hover:text-slate-300"}`} />
            ...
            {/* Logout */}
                <div className="p-3 border-t border-white/6">
                    <button onClick={handleLogout} className='flex items-center gap-3 w-full px-3 py-2.5 rounded-md text-sm font-medium text-[13px] hover:text-rose-400 hover:bg-rose-500/8 transition-all duration-150'>
    ...
    return (
        <>
            ...
            {/* Sidebar - desktop */}
             ...
             {/* Sidebar - mobile */}
             <aside className={`lg:hidden inset-y-0 left-0 w-72 bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 text-white z-50 flex flex-col transform transition-transform duration-300 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                {sidebarcontent}
             </aside>
